...
--filter-tcp=443 --hostlist="%LISTS%list-general.txt" --dpi-desync=multisplit --dpi-desync-split-seqovl=512 --dpi-desync-split-pos=2 --dpi-desync-split-seqovl-pattern="%BIN%4pda.bin" --dpi-desync-repeats=2 --new ^
...
--filter-tcp=443,13000,14000,14008,14020-14024,%GameFilter% --ipset="%LISTS%ipset-all.txt" --dpi-desync=multisplit --dpi-desync-split-seqovl=512 --dpi-desync-split-pos=2 --dpi-desync-split-seqovl-pattern="%BIN%4pda.bin" --dpi-desync-repeats=2 --new ^
--filter-udp=3074,3478-3480,3659,1000-20000,%GameFilter% --ipset="%LISTS%ipset-all.txt" --dpi-desync=fake --dpi-desync-autottl=2 --dpi-desync-repeats=8 --dpi-desync-any-protocol=1 --dpi-desync-fake-unknown-udp="%BIN%quic_initial_www_google_com.bin" --dpi-desync-cutoff=n3 --new